...
// Swaps the full `amountRaw` of `inputMint` back into SOL, signed by the core wallet.
export async function sellToken(inputMint, amountRaw, slippageBps = 300) {
  const wallet = coreWallet();
  const quote = await getQuote(inputMint, SOL_MINT, amountRaw, slippageBps);
  const r = await fetch(`${JUP}/swap`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({
